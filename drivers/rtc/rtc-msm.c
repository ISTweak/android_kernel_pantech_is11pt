...
#define USE_AUTOMATIC_UPDATE_RTC
#endif
...
#ifdef USE_AUTOMATIC_UPDATE_RTC
static int is_auto_update(void)
{
    int fd=-1;
    char buf[1]={0};
    const char* filename="/data/data/com.android.settings/files/auto_time.conf";
    int tries = 16 ;
	int ret = 0;
    ...
    if( fd < 0 ) {
        printk(KERN_INFO "open failed %s", filename) ;
		buf[0] = 0x30;
        goto fin ;
    }
	do{
		ret = sys_read(fd,(char *)buf,1);
		if(ret == 1)
		 goto fin;
	}while(--tries != 0);
	if(tries == 0) buf[0] = 0x30;
fin:
    if(fd) sys_close(fd);
    set_fs(old_fs);
    return buf[0] == 0x30 ? 0 : 1 ;
}
#endif // USE_AUTOMATIC_UPDATE_RTC
