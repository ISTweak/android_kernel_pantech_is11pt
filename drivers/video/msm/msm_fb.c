...
#define INIT_IMAGE_FILE "/power_on_02_1.rle"
#elif defined(CONFIG_MACH_MSM8X55_LASER2)
#define INIT_IMAGE_FILE "/laser2_poweron_01.rle"
#elif defined(CONFIG_MACH_MSM8X55_J_MASAI_E)
#define INIT_IMAGE_FILE "/poweron_logo.rle"
#else
...
short sky_lcdc_flag = 0;
//static int sky_loadbar_ratio = 0;
#endif

#ifdef CONFIG_MACH_MSM8X55_J_MASAI_E
extern void lcdc_masai_panel_off(int val);
#endif
#ifdef CONFIG_MACH_MSM8X55_LASER2
extern void mddi_laser_panel_off(void);
#endif



// 20110602, kkcho, LCD_OFF상태에서 reset이 걸렸을 경우, 부팅시 LCD가 ON되지 않도록 함.

#define F_SILENTBOOT_WITH_LCDOFF


#ifdef F_SILENTBOOT_WITH_LCDOFF
static int is_blind_reset = 0;
#endif

#ifdef F_SILENTBOOT_WITH_LCDOFF	
extern boolean sky_sys_rst_is_silent_boot_mode(void);
extern void sky_sys_rst_SetSilentBoot_bl_off(uint32_t mode);
...
//	mfd->bl_level = 0;
	mfd->bl_level = mfd->panel_info.bl_max; // shinbrad ( shinjg ) 
...
static int msm_fb_suspend_sub(struct msm_fb_data_type *mfd)
{
	...
	printk(KERN_INFO "msm_fb_suspend: called %d\n",mfd->suspend.panel_power_on);
...
static int msm_fb_resume_sub(struct msm_fb_data_type *mfd)
{
	...
	printk(KERN_INFO "msm_fb_resume: called %d\n",mfd->suspend.panel_power_on);
...
void msm_fb_set_backlight(struct msm_fb_data_type *mfd, __u32 bkl_lvl)
{
	...
	if (is_blind_reset) {
		printk("%s: [PANTEC_LCD] blind reset! return!\n",__func__);
		return;
	}
#endif
	printk("[%s:ENTER] bkl_lvl = %d\n",__func__,bkl_lvl);
	if ((pdata) && (pdata->set_backlight)) {
		down(&mfd->sem);
		mfd->bl_level = bkl_lvl;
		pdata->set_backlight(mfd);
		printk("[%s:PROGRESS] blk_lvl = %d \n",__func__,bkl_lvl);
		up(&mfd->sem);
	}
	printk("[%s:EXIT] \n",__func__);
}
#ifdef F_SILENTBOOT_WITH_LCDOFF
void msm_reset_set_bl_off(bool bl);
#endif
static int msm_fb_blank_sub(int blank_mode, struct fb_info *info,
			    boolean op_enable)
{
	...
	if (!op_enable)
	{
		printk(KERN_ERR "[PANTECH_LCD]%s: Incorrect code : top_enable=%d\n",__func__, op_enable);
		return -EPERM;
	}

	printk(KERN_ERR "[PANTECH_TRACE]%s: blank_mode=%d, mfd->panel_power_on=%d\n",__func__, blank_mode, mfd->panel_power_on);
	...
	case FB_BLANK_UNBLANK:
		if (!mfd->panel_power_on) {
			...
			if (ret == 0) {
                         if (mfd) {    /* shinbrad patch  backlight on after mdp on */
				mfd->panel_power_on = TRUE;

#ifdef F_SILENTBOOT_WITH_LCDOFF 
				if(!sky_sys_rst_is_silent_boot_mode())  // BLIND_RESET_TEMP event 1
					msm_reset_set_bl_off(0);  // LCD_ON
#endif			
					msm_fb_set_backlight(mfd, mfd->bl_level);
					printk(KERN_INFO "%s : called %d, mfd->bl_level = %d\n",__func__,blank_mode,mfd->bl_level);
                         }
...
*/
	...
	default:
		if (mfd->panel_power_on) {
			int curr_pwr_state;
#ifdef CONFIG_MACH_MSM8X55_J_MASAI_E
			lcdc_masai_panel_off(0);
#endif
#ifdef CONFIG_MACH_MSM8X55_LASER2
			mddi_laser_panel_off();
#endif
...
#if 1
// defined(CONFIG_MACH_MSM8X55_MINI)
			ret = pdata->off(mfd->pdev);	
#ifdef F_SILENTBOOT_WITH_LCDOFF 

			msm_reset_set_bl_off(1);  // LCD_OFF
#endif
...
int msm_reset_get_bl_off(void)
{
	int bl;
	bl = sky_sys_rst_GetSilentBoot_bl_off();  // 1: LCD-OFF, 0:LCD-ON
	printk(KERN_ERR "%s: reset_get_bl=%d\n",__func__, bl);
	return bl;
}
...
void msm_reset_set_bl_off(bool bl)
{
	...
	sky_sys_rst_SetSilentBoot_bl_off(bl); 
	result = sky_sys_rst_GetSilentBoot_bl_off();  
	printk("%s: reset_get_bl result =%d\n",__func__, result);	
}
#endif
static int msm_fb_register(struct msm_fb_data_type *mfd)
{
...
#if defined(CONFIG_MACH_MSM8X55_MINI) //chjeon20101228@PS2 chg temp (&240dpi(init.rc))
	var->height = 81.00,	/* height of picture in mm */
	var->width = 60.75, /* width of picture in mm */
#elif defined(CONFIG_MACH_MSM8X55_J_MASAI_E)
	...
	{
		if (msm_reset_get_bl_off() == 1){  // 1이면 LCD_OFF 상태
			is_blind_reset = 1;
			...
			load_565rle_image(INIT_IMAGE_FILE);	/* Flip buffer */
			is_blind_reset = 0;
			msm_fb_set_backlight(mfd, 15);
			printk(KERN_DEBUG "[%s]load_565rle_image input\n",__func__);
		}
		printk(KERN_DEBUG "[%s]###########################\n",__func__);
	}
	else{
...
static int msm_fb_release(struct fb_info *info, int user)
{
	...
	if (!mfd->ref_cnt) {
		printk(KERN_ERR "[PANTECH_LCD]%s: Imsm_fb_release: try to close unopened fb: mfd->index=%d\n",__func__, mfd->index);
