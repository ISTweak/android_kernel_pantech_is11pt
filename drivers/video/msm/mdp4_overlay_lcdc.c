...
int mdp_lcdc_on(struct platform_device *pdev)
{
	...
	int ret;
#ifdef F_SKYDISP_ADDITIONAL_MDP_LCDC_DMA_PATCH		
	struct clk *mdp_clk;  
#endif
	...
	if (is_mdp4_hw_reset()) {
		...
		printk(KERN_ERR "[PANTECH_LCD]%s: mdp_clk=%d\n",__func__, (int)clk_get_rate(mdp_clk));
#endif			
...
void mdp4_overlay_lcdc_wait4vsync(struct msm_fb_data_type *mfd)
{
	...
	spin_lock_irqsave(&mdp_spin_lock, flag);
	mdp_enable_irq(MDP_OVERLAY0_TERM);	  /* enable intr */
	INIT_COMPLETION(lcdc_comp);
