...
{
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_ON, FALSE);
	outpdw(MDP_BASE + 0x10100, 0x100); // MDP_LAYERMIXWE_IN_CFG 
	outpdw(MDP_BASE + 0x18000, 0x11); // MDP_OVERLAY_REG_FLUSH
	mdp_pipe_ctrl(MDP_CMD_BLOCK, MDP_BLOCK_POWER_OFF, FALSE);
}

void force_mdp4_overlay_control(bool onoff)
{
	screen_hold_state = onoff;
}

EXPORT_SYMBOL(force_mdp4_overlay_control);
