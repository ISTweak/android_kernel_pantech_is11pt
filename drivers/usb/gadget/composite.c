...
/* MTP authority */
#if defined(CONFIG_MACH_MSM8X55_MINI) || defined(CONFIG_MACH_MSM8X55_LASER2) || defined(CONFIG_MACH_MSM8X55_J_MASAI_E)
static DEVICE_ATTR(enable, S_IRUGO | S_IWUSR, enable_show, enable_store);
#else
static DEVICE_ATTR(enable, S_IRWXUGO, enable_show, enable_store);
#endif
...
void usb_composite_force_reset(struct usb_composite_dev *cdev)
{
	...
	if (cdev && cdev->gadget && cdev->gadget->speed != USB_SPEED_UNKNOWN) {
		...
		msleep(100);  //no response on modem port. 10 -> 100
