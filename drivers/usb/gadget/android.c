...
#if defined(CONFIG_PANTECH_USB) //msy_20110511 
...
//msy_20110511 USB serial uses BSN
static int android_pantech_set_serial_number(void)
{
  ...
  if(*ret.out_len <= 0)
  {
    ...
    usb_pr_info("## *ret.out_len <= 0 \n");
    return 0;
  }

#ifdef T_J_MASAI_E
  memcpy(pantech_serial_number, OEM_PRODUCT_DEVICE, strlen(OEM_PRODUCT_DEVICE));
  memcpy(pantech_serial_number + strlen(OEM_PRODUCT_DEVICE), ret.output, *ret.out_len);
#else
  ...
  usb_pr_info("## pantech_serial_number : %s \n", pantech_serial_number);  
  oem_rapi_client_close();
...
static int product_matches_functions(struct android_usb_product *p)
{
	...
	list_for_each_entry(f, &android_config_driver.functions, list) {  /*7 噺 欠覗 ,  rndis + diag + adb + modem + nmea + rmnet + ums*/
		if (product_has_function(p, f) == !!f->disabled) /*usb_product亜 戚 function聖 亜走壱 責澗汽, 益 function戚 disable 鞠嬢 責陥檎 獲 鎧亜 達澗 蕉亜 焼艦陥.*/
			return 0;                      /*usb_product亜 戚 function聖 亜走壱 蒸澗汽, 益 function戚 enable 鞠嬢 責陥檎 獲 鎧亜 達澗 蕉亜 焼艦陥.*/
	}
	return 1;
}
...
void android_enable_function(struct usb_function *f, int enable)
{
  ...
  usb_pr_info("## %s:%d enabel:%d name:%s\n", __FUNCTION__, __LINE__, enable, f->name);
...
//msy_20110511 USB serial uses BSN
#if defined(CONFIG_PANTECH_USB)
	if(device_desc.idVendor == PANTECH_VENDOR_ID)
	{
			pantech_serial_number = NULL;
