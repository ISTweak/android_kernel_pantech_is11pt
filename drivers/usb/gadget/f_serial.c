...
static void gser_disable(struct usb_function *f)
{
  ...
  usb_pr_info("## %s:%d generic ttyGS%d deactivated\n", __FUNCTION__, __LINE__, gser->port_num);		
...
static int gser_notify_serial_state(struct f_gser *gser)
{
  ...
  usb_pr_info("## %s:%d gser ttyGS%d serial state %04x\n", 
    __FUNCTION__, __LINE__, gser->port_num, gser->serial_state);		
