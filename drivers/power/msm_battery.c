...
#define BATTERY_QUERY_BATTERY_PARAMS_PROC 31
#endif
...
#if defined(CONFIG_PANTECH_PMIC8058)
struct msm_batt_get_batt_chg_req_data {
  u32 more_data;
};

struct msm_batt_get_batt_chg_rsp_data {
  u32 more_data;
  /*! The charger status             */
  u32 charger_status;  
  /*! The charger hardware type      */
  u32 charger_hardware_type; 
  /*! The battery status             */
  u32 battery_status; 
  /*! The battery level              */
  u32 battery_level;
...
#if defined(CONFIG_PANTECH_PMIC8058)
static struct msm_batt_get_batt_chg_rsp_data rsp;

static int msm_batt_get_batt_chg_arg_func(struct msm_rpc_client *batt_client,
				       void *buf, void *data)
{
	struct msm_batt_get_batt_chg_req_data *batt_read_req =
		(struct msm_batt_get_batt_chg_req_data *)data;
	u32 *req = (u32 *)buf;
	int size = 0;

	*req = cpu_to_be32(batt_read_req->more_data);
	size += sizeof(u32);
...
static int msm_batt_get_batt_chg_ret_func(struct msm_rpc_client *batt_client,
				       void *buf, void *data)
{
	struct msm_batt_get_batt_chg_rsp_data *data_ptr, *buf_ptr;

	data_ptr = (struct msm_batt_get_batt_chg_rsp_data *)data;
	buf_ptr = (struct msm_batt_get_batt_chg_rsp_data *)buf;

	if (be32_to_cpu(buf_ptr->more_data)) {
	  data_ptr->charger_status        = be32_to_cpu(buf_ptr->charger_status);
		data_ptr->charger_hardware_type = be32_to_cpu(buf_ptr->charger_hardware_type);
		data_ptr->battery_status        = be32_to_cpu(buf_ptr->battery_status);
    ...
    data_ptr->battery_temp          = be32_to_cpu(buf_ptr->battery_temp);
		data_ptr->is_charger_valid      = be32_to_cpu(buf_ptr->is_charger_valid);
    data_ptr->is_charging           = be32_to_cpu(buf_ptr->is_charging);
    data_ptr->is_battery_valid      = be32_to_cpu(buf_ptr->is_battery_valid);
    data_ptr->ui_event              = be32_to_cpu(buf_ptr->ui_event);
...
{
  #if defined(CONFIG_PANTECH_PMIC8058)
  struct msm_batt_get_batt_chg_req_data req;
	...
	rc = msm_rpc_client_req(msm_batt_info.batt_client,
		  BATTERY_QUERY_BATTERY_PARAMS_PROC,
		  msm_batt_get_batt_chg_arg_func, &req,
			msm_batt_get_batt_chg_ret_func, &rsp,
			msecs_to_jiffies(BATT_RPC_TIMEOUT));

	if (rc < 0) {
		pr_err("%s : FAIL. battery Charging Status RPC error = %d, \n",
				__func__, rc);
		return rc ;
	}
...
static void msm_batt_update_psy_status(void)
{
  ...
  u32 battery_soc;
	...
	charger_status   = rsp.charger_status;;
	charger_type     = rsp.charger_hardware_type;
	battery_status   = rsp.battery_status;
	battery_level    = rsp.battery_level;
	battery_voltage  = rsp.battery_voltage;
	battery_temp     = rsp.battery_temp;
  is_charger_valid = rsp.is_charger_valid;
	is_charging      = rsp.is_charging;
	is_battery_valid = rsp.is_battery_valid;
	ui_event         = rsp.ui_event;
	battery_soc      = rsp.battery_soc;
	...
	pr_info("BATT: CS %d, CT %d, BS %d, BL %d; BV %d, BT %d, ICV %d, IC %d, IBV %d, UE %d, SOC %d\n",
		 charger_status, charger_type, battery_status,
		 battery_level, battery_voltage, battery_temp, is_charger_valid, is_charging,
		 is_battery_valid, ui_event, battery_soc);

	if (battery_status == BATTERY_STATUS_INVALID &&
	    battery_level != BATTERY_LEVEL_INVALID) {
		pr_info("BATT: change status(%d) to (%d) for level=%d\n",
  ...
  msm_batt_info.batt_capacity = battery_soc;
  pr_info("BATT: voltage = %u mV [capacity = %d%%]\n",
