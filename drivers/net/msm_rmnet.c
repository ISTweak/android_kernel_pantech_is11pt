...
//MIRACHJ,20110801,twnam..added & moved here for 3G Reset by PC Crash Case Number: 00575682
	...
	for (;;) {
		...
		if (RMNET_IS_MODE_IP(opmode) ? (sz > dev->mtu) :
			...
			if (skb == NULL) {
				...
				//MIRACHJ,20110801,twnam..add for 3G Reset by PC Crash Case Number: 00575682
				/* out of memory, reschedule a later attempt */
				pr_err("rmnet_recv() debug msg check..(1)\n");
				printk("## NK [%s:%d] preempt_count : 0x%08x\n", __func__, __LINE__, preempt_count());
				pr_err("## NK [%s:%d] preempt_count : 0x%08x\n", __func__, __LINE__, preempt_count());
...
//MIRACHJ,20110801,twnam..moved upper for 3G Reset by PC Crash Case Number: 00575682
//static DECLARE_TASKLET(smd_net_data_tasklet, smd_net_data_handler, 0);
