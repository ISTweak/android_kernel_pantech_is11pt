...
static int sensor_get_debug(struct device *dev)
{
	...
	debug = atomic_read(&sensordata->debug);
...
static void sensor_set_debug(struct device *dev, int debug)
{
	struct input_dev *inputdev = to_input_dev(dev);
    struct sensor_data *sensordata = input_get_drvdata(inputdev);
	...
	atomic_set(&sensordata->debug, debug);
...
static ssize_t sensor_debug_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    mutex_lock(&sensordata->debug_mutex);

    debug = sensor_get_debug(dev);

    mutex_unlock(&sensordata->debug_mutex);
...
static DEVICE_ATTR(debug, S_IRUGO|S_IWUSR|S_IWGRP,
        sensor_debug_show, sensor_debug_store);
...
static int sensor_measure(struct sensor_data *data, axes_t *val)
{
	...
	debug = atomic_read(&data->debug);

	if(debug) {
		rc = apds9900_ls_measure(val);
	}
	else {
		rc = apds9900_ls_measure(&tmp);

		if(tmp.x < 15) val->x = 10;
		else if(tmp.x < 60) val->x = 50;
		else if(tmp.x < 250) val->x = 200;
		else if(tmp.x < 900) val->x = 500;
		else val->x = 1000;

		val->y = 0;
		val->z = 0;

		dbg("ambientlight.%s : (%d,%d,%d) -> (%d,%d,%d)\n", __func__, tmp.x, tmp.y, tmp.z, val->x, val->y, val->z);
	}
...
static int sensor_probe(struct platform_device *pdev)
{
	...
	mutex_init(&sensordata->debug_mutex);
