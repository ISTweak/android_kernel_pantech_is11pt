...
#define GEOMAGNETIC_SENSOR_EARLYSUSPEND // P12095. 110816 for google map application's sleep

#if defined(GEOMAGNETIC_SENSOR_EARLYSUSPEND) && defined(CONFIG_HAS_EARLYSUSPEND)
#include <linux/earlysuspend.h>
static struct early_suspend es;
static atomic_t disable_forced = ATOMIC_INIT(0);
#endif // GEOMAGNETIC_SENSOR_EARLYSUSPEND && CONFIG_HAS_EARLYSUSPEND
...
#if 1 // add board config
#define GEOMAGNETIC_GPIO_RESET	177
#define GEOMAGNETIC_VREG_STR	"gp4"
#define GEOMAGNETIC_VREG_VOLT	2600
#else
#error "[yas529.c] BOARD_VER is not defined."
#endif
...
#define MAGNETOMETER_POSITION 6
#elif defined(CONFIG_MACH_MSM8X55_J_MASAI_E)
#define MAGNETOMETER_POSITION 3
#elif defined(CONFIG_MACH_MSM8X55_LASER2)
#if (BOARD_VER == WS10 )
#define MAGNETOMETER_POSITION 5
#else
#define MAGNETOMETER_POSITION 0
#endif
#endif
...
void geomagnetic_set_power(bool on)
{
	struct vreg *vr;
	int rc;
	...
	vr = vreg_get(NULL, GEOMAGNETIC_VREG_STR);
	if(on) {
		rc = vreg_set_level(vr, GEOMAGNETIC_VREG_VOLT);
		if(rc){
			printk(KERN_ERR "%s: vreg %s set level failed (%d)\n", __func__, GEOMAGNETIC_VREG_STR, rc);
		}
		else {
			rc = vreg_enable(vr);
			if (rc) {
...
#ifdef POWER_ON_OFF
EXPORT_SYMBOL(geomagnetic_set_power);
#endif
void geomagnetic_set_reset(bool enable)
{
	unsigned cfg = GPIO_CFG(GEOMAGNETIC_GPIO_RESET, 0, GPIO_CFG_OUTPUT, GPIO_CFG_PULL_DOWN, GPIO_CFG_2MA);
	gpio_tlmm_config(cfg, GPIO_CFG_ENABLE);

	if(enable) {
		gpio_set_value(GEOMAGNETIC_GPIO_RESET, 1);
		mdelay(5);
		gpio_set_value(GEOMAGNETIC_GPIO_RESET, 0);
		mdelay(5);
		gpio_set_value(GEOMAGNETIC_GPIO_RESET, 1);
		mdelay(5);
	}
...
//	geomagnetic_set_reset(1);
    ...
    if (hwdep_driver.set_position != NULL) {
        if (hwdep_driver.set_position(MAGNETOMETER_POSITION) < 0) {
	...
	dbg_func_out();
	#ifdef POWER_ON_OFF
	printk("geomagnetic_set_power start\n");
	geomagnetic_set_power(0);
	printk("geomagnetic_set_power end\n");
	#endif
