	...
	if(timer_enabled)
	{
		if(timer_tick > 0) 
		{
			...
			{				
				...
				//ret = WRITE_MEM_FAILED; 
			}
			printk("[TSP] calibration really good\n");
	...
	{   	
		for ( i= 0; i<MAX_NUM_FINGER; i++ )
		{
			...
			if(fingerInfo[i].mode == TSC_EVENT_NONE)
			{
				...
				{
					printk("[TSP] Finger[%d] report event Down :(%d, %d)\n", i, fingerInfo[i].x, fingerInfo[i].y);
			...
			else
			{
				if (fingerInfo[i].status == 0)
				{
					if(fingerInfo[i].mode == TSC_EVENT_WINDOW)
					{
						...
						printk("[TSP] Finger[%d] report event Up :(%d, %d)\n",i, fingerInfo[i].realx, fingerInfo[i].realy);
						input_report_abs(qt602240_data->input_dev, ABS_MT_POSITION_X, fingerInfo[i].realx);
						input_report_abs(qt602240_data->input_dev, ABS_MT_POSITION_Y, fingerInfo[i].realy);
				...
				else if(fingerInfo[i].mode == TSC_EVENT_WINDOW)
				{
					...
					if(fingerInfo[i].status == 1 && (fingerInfo[i].y >= 800 && fingerInfo[i].y < TOUCH_KEY_Y)) {
...
int init_hw_setting(void)
{
	...
	int rc; 
	unsigned gpioConfig;
	struct vreg *vreg_touch_GP11,*vreg_touch_GP13;

	vreg_touch_GP11 = vreg_get(NULL, VDD_TOUCH_ID_GP11);
	rc = vreg_set_level(vreg_touch_GP11, VDD_TOUCH_MV_GP11);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP11 set level failed (%d)\n", __func__, rc);
		return -1;
	}

	rc = vreg_enable(vreg_touch_GP11);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP11 enable failed (%d)\n", __func__, rc);
		return -1;
	}
  
  vreg_touch_GP13 = vreg_get(NULL, VDD_TOUCH_ID_GP13);
	rc = vreg_set_level(vreg_touch_GP13, VDD_TOUCH_MV_GP13);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP13 set level failed (%d)\n", __func__, rc);
		return -1;
	}

	rc = vreg_enable(vreg_touch_GP13);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP13 enable failed (%d)\n", __func__, rc);
		return -1;
	}
	...
	if (rc) {
		dbg(KERN_ERR "%s: GPIO_TOUCH_RST failed (%d)\n",__func__, rc);
		return -1;
	}      
	...
	rc = gpio_tlmm_config(gpioConfig, GPIO_CFG_ENABLE);
	if (rc) {
		dbg(KERN_ERR "%s: GPIO_TOUCH_CHG failed (%d)\n",__func__, rc);
		return -1;
	}        
	TSP_Restart();
	msleep(100);
...
{ 
  int rc; 
	...
	rc = vreg_disable(vreg_touch_GP11);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP11 enable failed (%d)\n", __func__, rc);
		return;
	}
  ...
  vreg_touch_GP13 = vreg_get(NULL, VDD_TOUCH_ID_GP13);
	...
	rc = vreg_disable(vreg_touch_GP13);
	if (rc) {
		printk(KERN_ERR "%s: vreg_GP13 enable failed (%d)\n", __func__, rc);
		return;
	}
...
uint8_t QT_Boot(bool withReset)
{
	...
	unsigned char	*firmware_data = QT602240_E_firmware;
