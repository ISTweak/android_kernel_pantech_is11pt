...
static uint8_t qt_charger_mode_config(unsigned long mode)
{
    ...
    dbg("[TSP] qt_charger_mode_config [charger_mode=%ld, pre_charger_mode=%ld] \n", mode, pre_charger_mode);
...
*
* ***************************************************************************/
uint8_t post_calibrate_chip(void)
{
...
printk("\n");
	...
	write_mem(address+4, 1, &data); //AUTOCAL
	write_mem(address+6, 1, &data);	//ATCHCALST
	write_mem(address+7, 1, &data);	//ATCHCALTHR
	write_mem(address+8, 1, &data);	//ATCHFRCCALTHR
	write_mem(address+9, 1, &data);	//ATCHFRCALRATIO

	address = get_object_address(TOUCH_MULTITOUCHSCREEN_T9, instance);

	data = 30;

	write_mem(address+7, 1, &data);	//TCHTHR
...
uint8_t recovery_calibrate_chip(void)
{
	...
	data = T8_ATCHCALSTHR;
	write_mem(address+7, 1, &data);	//ATCHCALTHR
	data = T8_ATCHFRCCALTHR;
	write_mem(address+8, 1,  &data);	//ATCHFRCCALTHR
	data = T8_ATCHFRCCALRATIO;
	write_mem(address+9, 1, &data);	//ATCHFRCALRATIO
...
int8_t check_chip_calibration(void)
{
	...
	printk("[TSP] max_tch %d,  max_attch %d \n",max_tch, max_attch);
	printk("[TSP] Flags Counted channels: t:%d a:%d \n", tch_ch, atch_ch);
		
	if((tch_ch > max_tch ) || (atch_ch > max_attch))   // Palm state?
	{
		printk("[TSP]  maybe Palm state !!! \n");
	...
	else if(tch_ch + atch_ch > max_tch)        //unknown state
	{
		printk("[TSP]  maybe Palm state (tch_ch+atch_ch) !!! \n" );
	...
	{
		printk("[TSP]  Original anti-touch  !!! \n" );
	...
	else if((tch_ch) && (atch_ch == 0))
	{
		printk("[TSP]  Original touch  (tch_count = %d)!!! \n",good_touch_cnt );
		...
		if(autocal_cnt > 5) 
		{
			...
			data = 0;
			write_mem(address+4,1,&data);

			recovery_calibrate_chip();
		}

		if(good_touch_cnt > 20) 
		{
			printk("[TSP]  good_touch_cnt > 20  !!! \n" );
			recovery_calibrate_chip();
		}
	...
	else if(ABS(tch_ch ,atch_ch) < 10)
	{
		printk("[TSP]  calibration maybe good (thc+atch = %d), (autocal_cnt= %d)!!! \n",ABS(tch_ch ,atch_ch),autocal_cnt);
...
#if 0	
		if(autocal_cnt == 0) {
			printk("[TSP]  AUTOCAL Enable  !!! \n" );
			
			address = get_object_address(GEN_ACQUISITIONCONFIG_T8, instance);
			
			write_mem(address+4,1,&autocal_time);
			...
			printk("[TSP] autocal_cnt != 0  !!! \n" );
			autocal_cnt++;
			
			if(autocal_cnt > 20) 
			{
#endif
				if(tch_ch > 4  &&  atch_ch > 3){
					 calibrate_chip();
				}else{
				recovery_calibrate_chip();
				}
...
//    int8_t touch_ch,anti_ch;
    ...
    if (driver_setup == DRIVER_SETUP_OK)
    {
        ...
        if(read_mem(message_processor_address, max_message_length, quantum_msg) == READ_MEM_OK)
        {
            ...
            { 
				if( quantum_msg[1] & (TOUCH_DETECT|TOUCH_PRESS|TOUCH_MOVE))                                
				{
		  ...
		  /* Detect & Press */
...
#ifdef QT_MULTITOUCH_ENABLE
                if ( quantum_msg[1] & (TOUCH_RELEASE|TOUCH_SUPPRESS) )     
                {
            ...
            else if (object_type == GEN_COMMANDPROCESSOR_T6) 
            {
				...
				if((quantum_msg [1] & 0x10))
				{
					if(cal_check_flag == 0)
					{
						printk("[TSP] unknown calibration !!! \n");
						post_calibrate_chip();
					}
				}
            ...
            {
                    dbg("[TSP] 2222 msg id =  %d, status = 0x%x \n", quantum_msg[0],quantum_msg[1]);
					          if(quantum_msg[0] == 255 && quantum_msg[1] == 0x90){
						           check_chip_calibration();
				           	}
#if 0
...
//                	printk("[TSP] Finger[%d] Down(TSC_EVENT_WINDOW) XY(%d, %d)\n", i, fingerInfo[i].x, fingerInfo[i].y);
                ...
                else
                {
                     if (fingerInfo[i].status == 0)
                     {
                        if(fingerInfo[i].mode == TSC_EVENT_WINDOW)
                        {
                            ...
                            valid_area |= 1;
							if(fingerInfo[i].x > 167 && fingerInfo[i].x < 327 ){
									if(fingerInfo[i].y > 280 && fingerInfo[i].y < 581 ){
										//printk("^^^^ count_re %d\n",count_re);
										check_tch();
										if(count_re < 2){
											calibrate_chip();
    ...
    nPrevID= id;
#endif // #if defined(CONFIG_MACH_MSM8X55_J_MASAI_E)
