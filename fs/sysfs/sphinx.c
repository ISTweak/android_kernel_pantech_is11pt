...
/* KDDI Security OP pack beta2 porting 20101111 - evangelist */
...
#define CREATE_KOBJECT(dir, file)										\
	...
	static struct kobj_attribute dir##_attribute = __ATTR(file, 0664, NULL, NULL);	\
				...
				if(file)
				{
				  ...
				  if(p == NULL || (long)p == ENAMETOOLONG)continue;
					...
					if(vm_area->vm_flags & VM_MAYSHARE)
					{
						...
						{
/*  20110502 evangelist  : Gingerbread에서 state전환이 다름.... */	
#if 1
 					              if ( state1 != 100)						
 					              {
							memset(tmp, 0, PACKAGE_SHOW_MAX_LEN + 1);
							...
							apk_len = strlen(p);
							sphinx_printk("%s\n", tmp);
							sphinx_printk("527 state1 -> %d\n", state1);
							sphinx_printk("apk_len -> %d\n", apk_len);							
 					              }
 						      else
 						      	{
							goto JUMP; 						      	
							sphinx_printk("533 state1 -> %d\n", state1);	
 						      	}
#endif /* if 0 */
					...
					else
					{
						...
						if(ii == 0 && state0 == 0 && strstr(p, "/system/bin") == p)
						{
							...
							state1 = 100;
							// evangelist add msg
	                                          sphinx_printk("729 state0=[%d], state1=[%d]\n", state0,state1);
							break;
						}
					}
					...
					if(strstr(p, "/data") == p && strstr(p, "/data/dalvik-cache") == NULL && strstr(p, "/data/app") == NULL && strstr(p, ".apk") == NULL)
					{
						...
						// 20110602, SunggooLee, -start-	
						else if(strstr(p, ".db-shm")) 
						{                                          
							sphinx_printk("we don't need this item ebview.db-shm!!\n");
							goto SKIP_THIS; 	
						}
						// 20110602, SunggooLee, -end-	
						else
