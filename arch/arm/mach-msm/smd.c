...
{
      /*  UTF-8 
            ?„ìž¬??SYSTEM DOWNLOAD ëª¨ë“œë¥??´ìš©?´ì„œ 
             osbl??ì„œ message display ?˜ê³  normal boot ?˜ë?„ë¡??œë‹¤.
             release mode check , current action option
      */
...
#ifdef CONFIG_PANTECH_ERR_CRASH_LOGGING
      /* 
            when modem has crashed, back up linux debug infomation to smem (vendor2)
            1. kernel console log
            2. android logcat msg
            android pantech panic misc driver used.
      */
      oops_enter();
      spin_lock_irq(&modem_crash_lock);
      console_verbose();
      atomic_notifier_call_chain(&panic_notifier_list, 0, NULL);
      spin_unlock_irq(&modem_crash_lock);
      oops_exit();
      {
            regs = __get_regs_crashed();
            dst =(unsigned long *)(&(regs->uregs[4]));
            asm ("stmia   %0, {r4-r15}" : : "r" (dst) );
            dst =(unsigned long *)(&(regs->uregs[16]));
            asm ("mrs %0, cpsr" :: "r" (dst));
            flush_cache_all();
      }
      if(arm_crash_reset){
            arm_crash_reset();
