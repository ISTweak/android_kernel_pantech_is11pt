 ...
 * Restart Definitions
 *****************************************************************************/
//#define CONFIG_QUALCOMM_00543889_1
static uint32_t restart_reason = 0x776655AA;
#ifdef CONFIG_QUALCOMM_00543889_1
static DEFINE_SPINLOCK(msm_poweroff_lock);
#endif
static void msm_pm_power_off(void)
{
	msm_rpcrouter_close();
	msm_proc_comm(PCOM_POWER_DOWN, 0, 0);
	msm_proc_comm(PCOM_CUSTOMER_CMD3, 0, 0);
#ifdef CONFIG_QUALCOMM_00543889_1
	spin_lock_irq(&msm_poweroff_lock);
#endif
	for (;;)
 		 ;
...
#if defined(T_J_MASAI_E) || defined(T_LASER2)
...
} smem_id_vendor1_type;

static uint32_t rst_prev_reset;

static smem_id_vendor1_type *smem_id_vendor1_ptr;

static int oem_sys_read_proc_reset_info
	(char *page, char **start, off_t off, int count, int *eof, void *data)
{
	int len = 0;
  ...
  smem_id_vendor1_ptr = (smem_id_vendor1_type*)smem_alloc(SMEM_ID_VENDOR1,
	...
	len = sprintf(page, "SilentBoot: %d\n", smem_id_vendor1_ptr->silent_boot);

	printk(KERN_ERR "%s: rst_prev_silent_boot:%d\n", __func__,
	smem_id_vendor1_ptr->silent_boot);

	return len;
}

int oem_sys_write_proc_reset_info
...
void sky_sys_rst_set_smem_info(int *reason, int *reset, int *backlight, int* silent_boot)
{
  if(smem_id_vendor1_ptr == NULL)
  {
	...
	printk(KERN_WARNING"[%s]backlight =%d, silent_boot =%d\n",__func__,*backlight, *silent_boot);
  /*if(reason != NULL )
    smem_id_vendor1_ptr->reason = *reason;
    
  if(reset != NULL )
    smem_id_vendor1_ptr->reset = *reset;*/
    
  if(backlight != NULL )
    smem_id_vendor1_ptr->backlight_value = *backlight;
    
  if(silent_boot != NULL )
    smem_id_vendor1_ptr->silent_boot = *silent_boot;
}
EXPORT_SYMBOL(sky_sys_rst_set_smem_info);
...
bool sky_sys_rst_is_silent_boot_mode(void)
{
	...
	printk(KERN_WARNING"[%s]silent_boot =%d\n",__func__,smem_id_vendor1_ptr->silent_boot);

	if(smem_id_vendor1_ptr->silent_boot)
		return true;
...
void sky_sys_rst_SetSilentBoot_bl_off(uint32_t mode)
{
	if(smem_id_vendor1_ptr == NULL)
	{
		smem_id_vendor1_ptr = (smem_id_vendor1_type*)smem_alloc(SMEM_ID_VENDOR1, sizeof(smem_id_vendor1_type));
	...
	smem_id_vendor1_ptr->backlight_value = mode;
	printk("[%s:] smem_id_vendor1_ptr->backlightoff  = %d\n",__func__,smem_id_vendor1_ptr->backlight_value);
}
EXPORT_SYMBOL(sky_sys_rst_SetSilentBoot_bl_off);

bool sky_sys_rst_GetSilentBoot_bl_off(void)
{
	if(smem_id_vendor1_ptr == NULL)
	{
		smem_id_vendor1_ptr = (smem_id_vendor1_type*)smem_alloc(SMEM_ID_VENDOR1, sizeof(smem_id_vendor1_type));
	...
	if(smem_id_vendor1_ptr->backlight_value)  // 1: LCD-off, 0:LCD-on
		return true;//false;
	
	
	return  false;//true;
}
EXPORT_SYMBOL(sky_sys_rst_GetSilentBoot_bl_off);
...
// 20110216 jylee for bug report, /// wjmin2010.06.25@DS2 Added 
#if defined(T_J_MASAI_E) || defined(T_LASER2)
...
// 20110216 jylee for bug report, /// wjmin2010.06.25@DS2 Added 
	...
	if (reset_info) {
		reset_info->read_proc = oem_sys_read_proc_reset_info;
		reset_info->write_proc = oem_sys_write_proc_reset_info;
