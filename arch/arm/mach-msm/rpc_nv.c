...
//jwjeon1000701@DS2  //20100709 thkim_wifi CONFIG_PANTECH_WIFI_MAC_ADDR
#define NV_VERS_9_2		0x00090002 // 20110301 khlee_wifi newly added for runtime-error
...
static int msm_nv_check_validity(uint32_t item)
{
	...
	switch(item) {
		...
		case	NV_SPARE_2_I:
// 20110217 khlee_wifi kernel rpc work for reading wifi mac address from nvram
		case 	NV_WLAN_MAC_ADDRESS_I:  //20100628 thkim_wifi 
		
// CWG_system for IMEI setting ==>
		case 	NV_IMEI:
// CWG_system for IMEI setting ==<
				rc = 0;
				break;
...
// // CWG_system for IMEI setting ==>
		case	NV_IMEI:
				ptr->ue_imei[0]= cpu_to_be32(data->ue_imei[0]);	
				ptr->ue_imei[1]= cpu_to_be32(data->ue_imei[1]);	
				ptr->ue_imei[2]= cpu_to_be32(data->ue_imei[2]);
				
				*reqsz = req_hdr_sz + sizeof(nv_cmd_imei_req);
				*repsz = rep_hdr_sz + sizeof(nv_cmd_imei_rep);
				break;
// C// CWG_system for IMEI setting ==>
		// add here
	}
}
								...
								nv_cmd_item_type *data)
{
	...
	printk(KERN_INFO "msm_nv_set_rep_data 1\n");
	switch (item) {
		case	NV_SPARE_1_I:
...
// CWG_system for IMEI setting ==>
		case	NV_IMEI:	
				...
				data->ue_imei[0] = be32_to_cpu(ptr->ue_imei[0]);
				data->ue_imei[1] = be32_to_cpu(ptr->ue_imei[1]);		
				data->ue_imei[2] = be32_to_cpu(ptr->ue_imei[2]);
