...
#include "sky_snd_extampctrl_mini.h"  	//20110310 jhsong : sub chip cal tool by youngsoo Lim
...
static int snd_sub_debug_print = 0;  //20110629 jhsong
...
//	int status = 0;
	...
	if(snd_sub_debug_print) printk(KERN_ERR "[[ MAX97001 ]] ### extamp_dev_id(0x%x)\n", extamp_dev_id);
...
{
	if(snd_sub_debug_print) printk("[Audio] aud_sub_open \n");
	return 0;
}

static int aud_sub_release(struct inode *inode, struct file *file)
{
	if(snd_sub_debug_print) printk("[Audio] aud_sub_release \n");
	return 0;	
}
...
//		snd_subsystem_pmic_vreg_l2_on();	
	...
	if(snd_sub_debug_print) printk("[fab2200] fab2200_probe %d \n", rc);
	return rc;
}
...
static int __exit fab2200_remove(struct i2c_client *client)
{
	...
	if(snd_sub_debug_print) printk("[fab2200] fab2200_remove %d \n", rc);	
...
static int fab2200_suspend(struct i2c_client *client, pm_message_t mesg)
{
	if(snd_sub_debug_print) printk("[fab2200] fab2200_suspend\n");
	return 0;
}
...
static int fab2200_resume(struct i2c_client *client)
{
	if(snd_sub_debug_print) printk("[fab2200] fab2200_resume\n");
...
//	int status = 0;
	...
	if(snd_sub_debug_print) printk("[fab2200] fab2200_shutdown \n" );	
...
int snd_subsystem_get_Startgain_index(unsigned long long dB)
{
	...
	if(snd_sub_debug_print) printk("[fab2200] snd_subsystem_get_Startgain_index....... start_gain_index : %d \n", _start_gain_index);
...
int snd_subsystem_SetVolume(void)
{
	...
	case DEVICE_FMRADIO_HEADSET_RX:
		...
		tSubsystemInfo.diff_pga_info |= (/*0x00*/0x02<<PGAIN21_VALUE);  //PGA 0dB -> 6dB
			...
			tSubsystemInfo.hpl_vol_info = hpx_vol[/*25*/24]<<HPL_VOL_VALUE;  // 0dB -> -1dB (20110414 jhsong)
			tSubsystemInfo.hpr_vol_info = hpx_vol[/*25*/24]<<HPR_VOL_VALUE;  // 0dB -> -1dB (20110414 jhsong)
		}		
...
#endif		
		{
			start_gain_arr_index = snd_subsystem_get_Startgain_index(/*0*/4.5);  //0dB  ( -25dB ~ 6dB)
...
#endif
		{
			start_gain_arr_index = snd_subsystem_get_Startgain_index(/*6*/5.5/*5*/);  //5dB  ( -25dB ~ 6dB)
			tSubsystemInfo.start_gain_info = start_gain[start_gain_arr_index]<<START_GAIN_VALUE;
		}		
//		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);  //PGA 15dB
		status|=snd_subsystem_i2c_write(START_GAIN_REG,tSubsystemInfo.start_gain_info);
...
#endif
		{
			start_gain_arr_index = snd_subsystem_get_Startgain_index(/*4.5*/5.5);  //5dB  ( -25dB ~ 6dB)
			tSubsystemInfo.start_gain_info = start_gain[start_gain_arr_index]<<START_GAIN_VALUE;
		}		
...
//		return rc;
	...
	if(snd_sub_debug_print) printk("[fab2200] snd_subsystem_pmic_vreg_l2_on rc:%d\n", rc);
...
//		return rc;
	...
	if(snd_sub_debug_print) printk("[fab2200] snd_subsystem_pmic_vreg_l2_off rc:%d \n", rc);
...
static void snd_subsystem_en_init(void)
{
	...
	if(snd_sub_debug_print) printk("[fab2200] @@@snd_subsystem_en_init.....audio_sub_en GPIO init \n");
...
//	msm_snddev_poweramp_on();	//Set /RST pin to H (GPIO82)
	...
	if(snd_sub_debug_print) printk("[fab2200] @@@AMP start up \n");
}
...
//	int status = 0;
	...
	if(snd_sub_debug_print) printk("[fab2200] ###AMP disable \n");
...
void snd_subsystem_sp_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_sp_poweron............cal tool (status:%d)\n", status);
	}
#endif
...
void snd_subsystem_hp_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_hp_poweron ..............cal tool(status:%d)\n", status);
	}
#endif
...
void snd_subsystem_tty_hp_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_tty_hp_poweron.............cal tool (status:%d)\n", status);
	}
#endif
...
void snd_subsystem_sp_hp_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_sp_hp_poweron.............cal tool (status:%d)\n", status);
	}
#endif
...
void snd_subsystem_nr_ft_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_nr_ft_poweron...........cal tool (status:%d)\n", status);
	}
#endif
...
void snd_subsystem_nr_dv_poweron(void)
{
	...
	if(LockForTest){
		...
		if(snd_sub_debug_print) printk("[fab2200]snd_subsystem_nr_dv_poweron.............cal tool (status:%d)\n", status);
	}
#endif
...
void snd_subsystem_sp_powerdown(void)
{
	...
	printk("[fab2200] snd_subsystem_sp_powerdown (status:%d)\n", status);
...
//	status|= snd_subsystem_sp_ng_ssmt(SYSTEM_OFF);
	...
	printk("[fab2200] snd_subsystem_nr_ft_powerdown (status:%d)\n", status);
...
void snd_subsystem_nr_off_ft_powerdown(void)
{
	...
	printk("[fab2200] snd_subsystem_nr_off_ft_powerdown (status:%d)\n", status);
...
//	status|= snd_subsystem_sp_ng_ssmt(SYSTEM_OFF);
	...
	printk("[fab2200] snd_subsystem_nr_dv_powerdown (status:%d)\n", status);
...
static int snd_subsystem_common_system(int en)
{
	...
	if(snd_sub_debug_print) printk("[fab2200]COMMON SYSTEM(en:%d, reg_value:%d, status:%d)\n", en, reg_value, status);
...
static int snd_subsystem_sp_input_system(int en)
{
	...
	if(en){
		...
		if(LockForTest){
			...
			if(snd_sub_debug_print) printk("@#@#snd_subsystem_sp_input_system.....lockforTest......preGain : 0x%x",tSubsystemInfo.diff_pga_info);
		}else
#endif
		{
			tSubsystemInfo.diff_pga_info |= (/*0x00*/0x02<<PGAIN43_VALUE);  //PGA 15dB -> 0dB -> 6dB
			if(snd_sub_debug_print) printk("@#@#snd_subsystem_sp_input_system...........preGain : 0x%x",tSubsystemInfo.diff_pga_info);
		}
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
...
#endif
		{
			tSubsystemInfo.diff_pga_info |= (/*0x05*//*0x00*//*0x02*/0x03<<PGAIN43_VALUE);  //PGA 15dB -> 0dB -> 6dB
		}
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
...
#endif
		{
			tSubsystemInfo.diff_pga_info |= (/*0x02*/0x03<<PGAIN43_VALUE);  //PGA 15dB -> 0dB -> 6dB
		}
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
...
#endif
		{
			tSubsystemInfo.diff_pga_info |= (/*0x05*//*0x00*//*0x04*//*0x02*/0x03<<PGAIN43_VALUE);  //PGA 15dB -> 0dB -> 6dB
		}
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
...
{	
	...
	if(snd_sub_debug_print) printk("[fab2200] snd_subsystem_i2c_write %d \n", ret);
	return 0;
}
