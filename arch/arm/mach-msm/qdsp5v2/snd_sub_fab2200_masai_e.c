...
#include "sky_snd_extampctrl_masai_e.h"  //20110310 jhsong : sub chip cal tool by youngsoo Lim
...
static int snd_subsystem_voice_sp_input_system(int en);
static int snd_subsystem_voice_hp_input_system(int en);
...
static int snd_subsystem_sp_nr_ssmt(int en);
...
//	tSubsystemInfo_cal.power_info = (u8)DEFAULT_DATA_REG;
	...
	/* DIFFIN43(BIT7) DIFF21(BIT6) seamless(static) value */
	tSubsystemInfo_cal.diff_pga_info = (tSubsystemInfo_cal.diff_pga_info & 0xC0) |
	  (u8)dev_info->pga_info43<<PGAIN43_VALUE | (u8)dev_info->pga_info21<<PGAIN21_VALUE;
...
#ifdef CONFIG_PANTECH_MASAI_E_SEPERATE_CALL_MEDIA_PATH  //20110420 jhsong : seperate call and media path
		case DEVICE_SND_DEVICE_VOICE_SPEAKER_RX:
#endif
...
int snd_subsystem_SetVolume(void)
{
	...
	case DEVICE_HEADSET_RX:
	case DEVICE_FMRADIO_HEADSET_RX:
    di = extamp_lookup_device(0x02); // EXTAMP_HEADPHONE_STERE(0x02)O == DEVICE_HEADSET_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
		if(LockForTest)
	    di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
	  /* DIFFIN43 DIFF21(BIT7 BIT6 & 0x11000000(2)=0xC0 ) is static value */
		tSubsystemInfo.diff_pga_info = (tSubsystemInfo.diff_pga_info & 0xC0) | di->pga_info21<<PGAIN21_VALUE;
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
		
		tSubsystemInfo.hpl_vol_info = di->hpl_vol_info<<HPL_VOL_VALUE;
		status|=snd_subsystem_i2c_write(HPL_VOL_REG, (tSubsystemInfo.hpl_vol_info));
		
		tSubsystemInfo.hpr_vol_info = di->hpr_vol_info<<HPR_VOL_VALUE;
		status|=snd_subsystem_i2c_write(HPR_VOL_REG,(tSubsystemInfo.hpr_vol_info));
		break;
...
//		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);  //PGA 15dB
    di = extamp_lookup_device(0x01); // EXTAMP_SPKR_PHONE(0x01) == DEVICE_SPEAKER_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
    if(LockForTest)
      di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
    tSubsystemInfo.start_gain_info = (u8)di->start_gain_info<<START_GAIN_VALUE;
	...
	case DEVICE_SPEAKER_HEADSET_RX:
    printk("@#@#snd_subsystem_SetVolume...........DEVICE_SPEAKER_HEADSET_RX");
    di = extamp_lookup_device(0x03); // EXTAMP_SPKR_PHONE_AND_HP_STEREO(0x03) == DEVICE_SPEAKER_HEADSET_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
    if(LockForTest)
      di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
    /* DIFFIN43 DIFF21(BIT7 BIT6 & 0x11000000(2)=0xC0 ) is static value */
    tSubsystemInfo.diff_pga_info = (tSubsystemInfo.diff_pga_info & 0xC0)
      | ((di->pga_info43<<PGAIN43_VALUE) | (di->pga_info21<<PGAIN21_VALUE));
    status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
    
    tSubsystemInfo.start_gain_info = di->start_gain_info<<START_GAIN_VALUE;
    status|=snd_subsystem_i2c_write(START_GAIN_REG,tSubsystemInfo.start_gain_info);
...
#endif
		{
		tSubsystemInfo.diff_pga_info |= (/*0x00*/0x02<<PGAIN21_VALUE);  //PGA 0dB -> 6dB
...
#endif		
		{
			start_gain_arr_index = snd_subsystem_get_Startgain_index(0);  //0dB  ( -25dB ~ 6dB)
...
#endif
		{
			tSubsystemInfo.diff_pga_info |= ((/*0x00*/0x04<<PGAIN43_VALUE) | (/*0x05*/0x00<<PGAIN21_VALUE)); //PGA 15dB -> 0dB
		}
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);  
...
#endif		
		{
			start_gain_arr_index = snd_subsystem_get_Startgain_index(5);  //5dB  ( -25dB ~ 6dB)
			tSubsystemInfo.start_gain_info = start_gain[start_gain_arr_index]<<START_GAIN_VALUE;
		}
		status|=snd_subsystem_i2c_write(START_GAIN_REG,tSubsystemInfo.start_gain_info);
#ifdef FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
...
#if 1 //YSTEST
	case DEVICE_SND_DEVICE_VOICE_HEADSET_RX:
    di = extamp_lookup_device(0x07); // EXTAMP_HEADPHONE_STEREO_INCALL(0x07) == DEVICE_HEADSET_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
		if(LockForTest)
	    di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
...
** snd_subsystem_pmic_vreg_l2_on
**=========================================================================*/
static struct vreg *vreg_subsystem;

void snd_subsystem_pmic_vreg_l2_on(void)
{
	int rc;

//	vreg_subsystem = vreg_get(NULL, "xo_out");
	vreg_subsystem = vreg_get(NULL, "s3");  //20110208 jhsong

	rc = vreg_set_level(vreg_subsystem, 2000);
...
//		return rc;
	...
	rc = vreg_enable(vreg_subsystem);
	if (rc){
		printk(KERN_ERR "%s: vreg_enable failed(%s) = %d \n",
					__func__, "s3", rc);
//		return rc;
	}
	printk("[fab2200] snd_subsystem_pmic_vreg_l2_on rc:%d\n", rc);
...
void snd_subsystem_pmic_vreg_l2_off(void)
{
	...
	if (IS_ERR(vreg_subsystem)) {
		printk("%s: vreg_get(%s) failed (%ld)\n",
		__func__, "s3", PTR_ERR(vreg_subsystem));
//		return -1;
	}

	rc = vreg_disable(vreg_subsystem);
...
//		return rc;
	...
	vreg_put(vreg_subsystem);

	printk("[fab2200] snd_subsystem_pmic_vreg_l2_off rc:%d \n", rc);
...
** snd_subsystem_en_init
**=========================================================================*/
#define GPIO_NR_OUT_SEL 85   //20110207 jhsong : test
static void snd_subsystem_en_init(void)
{
	int rc;
...
//	msm_snddev_poweramp_on();	//Set /RST pin to H (GPIO82)

#if 1 //20110126 jhsong : to do after add gpio pin from hw
	gpio_set_value(GPIO_SUBSYSTEM_ENABLE, 1);	/* enable spkr poweramp */
#endif
...
#if 1 //20110126 jhsong : to do after add gpio pin from hw
	gpio_set_value(GPIO_SUBSYSTEM_ENABLE, 0);	/* disable spkr poweramp */
#endif
...
**=========================================================================*/
void snd_subsystem_sp_poweron(void)
{
	int status = 0;
	...
	status|= snd_subsystem_sp_nr_ssmt(SYSTEM_ON);
...
**=========================================================================*/
void snd_subsystem_sp_poweron_postProc(void)
{
	int status = 0;
	
	status|= snd_subsystem_SetVolume();
	
	printk("[fab2200]snd_subsystem_sp_poweron_postProc (status:%d)\n", status);
...
**=========================================================================*/
void snd_subsystem_hp_poweron(void)
{
	int status = 0;
	...
	status|=snd_subsystem_Set_HP_Noise_Gate(SYSTEM_OFF);
	status|=snd_subsystem_common_system(SYSTEM_ON);
...
**=========================================================================*/
void snd_subsystem_hp_poweron_postProc(void)
{
	int status = 0;

	status|=snd_subsystem_SetVolume();

	printk("[fab2200]snd_subsystem_hp_poweron_postProc (status:%d)\n", status);
...
#ifdef CONFIG_PANTECH_MASAI_E_SEPERATE_CALL_MEDIA_PATH  //20110420 jhsong : seperate call and media path
/*==========================================================================
** snd_subsystem_voice_sp_poweron
**=========================================================================*/
void snd_subsystem_voice_sp_poweron(void)
{
	int status = 0;
	...
	if(LockForTest){
		...
		printk("[fab2200]snd_subsystem_voice_sp_poweron............cal tool (status:%d)\n", status);
	}
#endif
	status|=snd_subsystem_voice_sp_input_system(SYSTEM_ON);
	status|= snd_subsystem_SetVolume();
	status|= snd_subsystem_sp_nr_ssmt(SYSTEM_ON);
	status|=snd_subsystem_common_system(SYSTEM_ON);
	
	printk("[fab2200]snd_subsystem_voice_sp_poweron (status:%d)\n", status);
...
**=========================================================================*/
void snd_subsystem_voice_hp_poweron(void)
{
	int status = 0;

	snd_subsystem_startup();

	status|=snd_subsystem_voice_hp_input_system(SYSTEM_ON);	
	status|=snd_subsystem_Set_HP_Noise_Gate(SYSTEM_OFF);
	status|=snd_subsystem_common_system(SYSTEM_ON);

	printk("[fab2200]snd_subsystem_voice_hp_poweron (status:%d)\n", status);
...
**=========================================================================*/
void snd_subsystem_voice_hp_poweron_postProc(void)
{
	int status = 0;
...
**=========================================================================*/
void snd_subsystem_tty_hp_poweron(void)
{
	int status = 0;
...
**=========================================================================*/
void snd_subsystem_sp_hp_poweron(void)
{
	int status = 0;

	snd_subsystem_startup();

#if 0//def FEATURE_SKY_SND_EXTAMPCTRL_FOR_APP    //20110310 jhsong : sub chip cal tool by youngsoo Lim
	if(LockForTest){
		snd_extamp_api_SetVolume_ById(EXTAMP_TEST_ID);
...
#if 0 //20110324 jhsong remove nr path for masai_e
/*==========================================================================
** snd_subsystem_nr_ft_poweron
**=========================================================================*/
void snd_subsystem_nr_ft_poweron(void)
{
	int status = 0;
...
**=========================================================================*/
void snd_subsystem_nr_off_ft_poweron(void)
{
	int status = 0;
...
**=========================================================================*/
void snd_subsystem_nr_dv_poweron(void)
{
	int status = 0;
...
**=========================================================================*/
void snd_subsystem_nr_off_dv_poweron(void)
{
	int status = 0;
...
void snd_subsystem_sp_powerdown(void)
{
	...
	// agc / nr set default
	status|= snd_subsystem_sp_nr_ssmt(SYSTEM_OFF);
...
#ifdef CONFIG_PANTECH_MASAI_E_SEPERATE_CALL_MEDIA_PATH  //20110420 jhsong : seperate call and media path
/*==========================================================================
** snd_subsystem_voice_sp_powerdown
**=========================================================================*/
void snd_subsystem_voice_sp_powerdown(void)
{
	int status = 0;

	// start gain mute
	snd_subsystem_SetMute(DEVICE_SND_DEVICE_VOICE_SPEAKER_RX, VOLUME_MUTE);

	// agc / nr set default
	status|= snd_subsystem_sp_nr_ssmt(SYSTEM_OFF);
	
	// input close
	status|= snd_subsystem_voice_sp_input_system(SYSTEM_OFF);
	...
	printk("[fab2200] snd_subsystem_voice_sp_powerdown (status:%d)\n", status);
...
/*==========================================================================
** snd_subsystem_voice_hp_powerdown
**=========================================================================*/
void snd_subsystem_voice_hp_powerdown(void)
{
	int status = 0;

	// hpl, hpr volume mute
	snd_subsystem_SetMute(DEVICE_SND_DEVICE_VOICE_HEADSET_RX, VOLUME_MUTE);

	// hp noise gate set default
	snd_subsystem_Set_HP_Noise_Gate(SYSTEM_OFF);
	//input close
	status|= snd_subsystem_voice_hp_input_system(SYSTEM_OFF);
	...
	printk("[fab2200]snd_subsystem_voice_hp_powerdown (status:%d)\n", status);
...
#ifdef CONFIG_PANTECH_MASAI_E_SEPERATE_CALL_MEDIA_PATH  //20110420 jhsong : seperate call and media path
			|| (snd_subsystem_get_device() == DEVICE_SND_DEVICE_VOICE_HEADSET_RX)
#endif
			)
		{
...
#if 1//YSTEST
	struct extamp_device_info *di;
	di = extamp_lookup_device(0x01); // EXTAMP_SPKR_PHONE == DEVICE_SPEAKER_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL  //20110310 jhsong : sub chip cal tool by youngsoo Lim
    if(LockForTest)
      di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
...
#if 1//YSTEST
    tSubsystemInfo.diff_pga_info |= (di->pga_info43<<PGAIN43_VALUE);  //PGA 15dB -> 0dB
...
#endif
		{
			tSubsystemInfo.diff_pga_info |= (/*0x05*/0x00<<PGAIN43_VALUE);  //PGA 15dB -> 0dB
			printk("@#@#snd_subsystem_sp_input_system...........preGain : 0x%x",tSubsystemInfo.diff_pga_info);
		}
#endif
		status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
...
#ifdef CONFIG_PANTECH_MASAI_E_SEPERATE_CALL_MEDIA_PATH  //20110420 jhsong : seperate call and media path
/*==========================================================================
** snd_subsystem_voice_sp_input_system
**=========================================================================*/
static int snd_subsystem_voice_sp_input_system(int en)
{
	int status = 0;
	...
	if(en){
		current_device = DEVICE_SND_DEVICE_VOICE_SPEAKER_RX;
...
/*==========================================================================
** snd_subsystem_voice_hp_input_system
**=========================================================================*/
static int snd_subsystem_voice_hp_input_system(int en)
{
	int status = 0;
	...
	if(en){
		current_device = DEVICE_SND_DEVICE_VOICE_HEADSET_RX;
...
#endif		
		{
			tSubsystemInfo.input_spk_hp_zc_info |= ((HP_ZCD_VALUE & BIT_SET_VALUE) 
														| (RAMP_SPEED_2MS<<HPRAMP_SPEED_VALUE) | (BIT_SET_VALUE&HP_RAMP_VALUE));  // hp zero cross detection enable
		}
	}else{
...
static int snd_subsystem_hp_input_system(int en)
{
	...
	struct extamp_device_info *di;
	di = extamp_lookup_device(0x02); // EXTAMP_HEADPHONE_STEREO(0x02) == DEVICE_HEADSET_RX
#ifdef FEATURE_SKY_SND_EXTAMPCTRL  //20110310 jhsong : sub chip cal tool by youngsoo Lim
  if(LockForTest)
    di = extamp_lookup_device(0xFF); // 0xFF == EXTAMP_TEST_ID
#endif
...
#else
    tSubsystemInfo.input_spk_hp_zc_info |= ((HP_ZCD_VALUE & BIT_SET_VALUE)
                            | (di->hp_ramp_speed<<HPRAMP_SPEED_VALUE) | (di->hp_ramp_en&BIT_SET_VALUE));  // hp zero cross detection enable
#endif
	  status|=snd_subsystem_i2c_write(INPUT_SPK_HP_ZC_REG, tSubsystemInfo.input_spk_hp_zc_info);
	}else{
		tSubsystemInfo.diff_pga_info &= BIT_CLEAR_VALUE;  // in2,1 configure to independent
...
static int snd_subsystem_sp_hp_input_system(int en)
{
    ...
    status|=snd_subsystem_i2c_write(DIFF_PGA_REG, tSubsystemInfo.diff_pga_info);
		...
		tSubsystemInfo.input_spk_hp_zc_info |= ((SEL_SPA4_VALUE & BIT_SET_VALUE) | (SEL_SPA3_VALUE & BIT_SET_VALUE) |(HP_ZCD_VALUE & BIT_SET_VALUE) 
                     | (di->hp_ramp_speed<<HPRAMP_SPEED_VALUE) | (BIT_SET_VALUE&HP_RAMP_VALUE));  // hp zero cross detection enable
    status|=snd_subsystem_i2c_write(INPUT_SPK_HP_ZC_REG, tSubsystemInfo.input_spk_hp_zc_info);
