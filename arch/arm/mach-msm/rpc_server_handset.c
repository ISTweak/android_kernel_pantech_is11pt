...
#include <mach/pmic.h>						// control hsed
#include <asm/mach-types.h>
#include <mach/vreg.h>
...
/* debug option */
/* -------------------------------------------------------------------- */
//#define EARJACK_DBG_ENABLE
#ifdef EARJACK_DBG_ENABLE
#define dbg_earjack(fmt, args...)   printk("[EARJACK]" fmt, ##args)
#else
#define dbg_earjack(fmt, args...)
#endif
#define dbg_earjack_func_in()       dbg_earjack("[FUNC_IN] %s\n", __func__)
#define dbg_earjack_func_out()      dbg_earjack("[FUNC_OUT] %s\n", __func__)
#define dbg_earjack_line()          dbg_earjack("[LINE] %d(%s)\n", __LINE__, __func__)
/* -------------------------------------------------------------------- */
...
static const uint32_t hs_key_map[] = {
	...
	KEY(HS_SKY_3P5PI_STEREO_HEADSET_K, SW_HEADPHONE_INSERT_W_MIC/*SW_HEADPHONE_INSERT*/), //3.5pi headset(mic)
	KEY(HS_SKY_3P5PI_STEREO_HEADSET_NO_MIC_K, /*SW_HEADPHONE_INSERT_W_MIC*/SW_HEADPHONE_INSERT), //3.5pi headset(no mic)
...
#endif
#ifdef CONFIG_MACH_MSM8X55_J_MASAI_E
static int is_earjack_connected = 0;
#endif
...
////////////////////////////////////////////////////////////////////////////////////
/*			LASER2 3.5PI EARJACK DRIVER											  */
////////////////////////////////////////////////////////////////////////////////////
#if defined (CONFIG_MACH_MSM8X55_LASER2) && (BOARD_VER > WS20)
static struct vreg *vreg_Earjack_GP6;
#endif

#if defined (CONFIG_MACH_MSM8X55_LASER2)
#define EARJACK_DET     40	 	/* Earjack sensing interrupt pin No. */
#define REMOTEKEY_DET   113     /*  */

//#define REMOTEKEY_DET_ACTIVE_LOW // select if remoteky_det pin is active low

#ifdef EARJACK_DET_ACTIVE_LOW
#define EARJACK_INSERTED !gpio_get_value(EARJACK_DET)
...
#define REMOTEKEY_PRESSED !gpio_get_value(REMOTEKEY_DET)
#else
#define REMOTEKEY_PRESSED  gpio_get_value(REMOTEKEY_DET)
#endif
...
//static void Is_inserted_earjack(void);

void enable_hsed(void){
#if (BOARD_VER > WS20 )
  vreg_enable(vreg_Earjack_GP6);  
#else
	pmic_hsed_enable(PM_HSED_CONTROLLER_1, PM_HSED_ENABLE_ALWAYS);	//ON MIC BIAS
#endif//(BOARD_VER > WS20 )
}
void disable_hsed(void){
#if (BOARD_VER > WS20 )
  vreg_disable(vreg_Earjack_GP6);
#else
	pmic_hsed_enable(PM_HSED_CONTROLLER_1, PM_HSED_ENABLE_OFF);	//ON MIC BIAS
#endif//(BOARD_VER > WS20 )
}
bool is_4pole_earjack (void) 
{	
	return !gpio_get_value(REMOTEKEY_DET);
}
void event_up(void)
{
  dbg_earjack("event_up Function start hs->hs_on %d, hs->mic_on %d\n",hs->hs_on, hs->mic_on);					
...
static irqreturn_t Earjack_Det_handler(int irq, void *dev_id)
{
	...
	dbg_earjack("Earjack_Det_handler \n");
...
{	
	dbg_earjack("currnet earjack->type: ");
	switch(hs->type){
		case    EARJACK_STATE_OFF : 
			{
				dbg_earjack("EARJACK_STATE_OFF\n");
				if(EARJACK_INSERTED){
					hs->type = EARJACK_STATE_CHECK;
					enable_hsed();
					schedule_delayed_work(&earjack_work, 15);
		...
		case    EARJACK_STATE_CHECK  :   
			{
				dbg_earjack("EARJACK_STATE_CHECK\n");   
				if(EARJACK_INSERTED)
				{
					// 3pole earjack is inserted
					if(!is_4pole_earjack()){					
					dbg_earjack("3POLE FIRST CHECK\n");
          hs->type= EARJACK_STATE_ON;
          hs->hs_on = 1 ;
					event_up();          
          disable_hsed();
					}
          //4pole earjack is inserted
					else {
						dbg_earjack("4POLE FIRST CHECK\n");
            hs->type= EARJACK_STATE_ON;
          ...
          //return without enable IRQ, wake_unlock.				
				...
				{
					dbg_earjack("earjack_type: -> EARJACK_STATE_OFF\n");
		...
		case EARJACK_STATE_ON:   
			{
				dbg_earjack("EARJACK_STATE_ON\n");
				if(EARJACK_RELEASED){
					dbg_earjack("EARJACK_RELEASED_ON\n");
					hs->type = EARJACK_STATE_OFF;
					// if 4pole
					if(hs->mic_on){            
						hs->mic_on = 0 ;
...
{
	dbg_earjack("Remotekey_Det_handler start\n");
	
	if(!(hs->mic_on) || EARJACK_RELEASED) {
		return IRQ_HANDLED;
	}
	disable_irq_nosync(gpio_to_irq(REMOTEKEY_DET));
	wake_lock_timeout(&remotekey_wake_lock,50);
	schedule_delayed_work(&remotekey_work, 3);
...
{
  dbg_earjack("remotekey_detect_func start\n");
	if(EARJACK_RELEASED){
    dbg_earjack("remotekey_detect_func start && EARJACK RELEASED RIGHT NOW\n");
		enable_irq(gpio_to_irq(REMOTEKEY_DET));		
    hs->remotekey_count=0;
		return;
	}
  if(hs->remotekey_count==1){     // 
    dbg_earjack("remotekey_detect_func CHECK TIME\n");
    hs->remotekey_count++;
    schedule_delayed_work(&remotekey_work, 6);      
    return;
  }
  if(!(hs->remotekey_pressed)){
    if(REMOTEKEY_PRESSED){
  		dbg_earjack("current state: remotekey pressed. \n");
  		hs->remotekey_pressed=1;      
      hs->remotekey_count=1;
      schedule_delayed_work(&remotekey_work, 6);      
      return;
    }
  	else{
      if(hs->remotekey_first){
    		dbg_earjack("current state: remotekey released. \n");
    		input_report_key(hs->ipdev, KEY_MEDIA, hs->remotekey_pressed);
  	    input_sync(hs->ipdev);
    ...
    if(REMOTEKEY_PRESSED){
      dbg_earjack("current state: remotekey long pressed start \n");
      input_report_key(hs->ipdev, KEY_MEDIA, hs->remotekey_pressed);
	    input_sync(hs->ipdev);
      hs->remotekey_pressed=0;
      hs->remotekey_first=1;
      hs->remotekey_count=0;
    }else{
      dbg_earjack("current state: remotekey pressed & released \n");
      input_report_key(hs->ipdev, KEY_MEDIA, hs->remotekey_pressed);
	    input_sync(hs->ipdev);
      hs->remotekey_pressed=0;
      hs->remotekey_count=0;
